#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Assemble batch-09.tsv col4 from _f09/*.out.tsv (linenum<TAB>chinese).
Keyed by source LINE NUMBER (robust to duplicate keys). Cols 1-3 byte-preserved.
Validates: row/col count, placeholder multiset (case-sensitive), LINK open/close
byte-exact parity, no real tab/newline in translation."""
import sys, re, os
BASE=os.path.dirname(os.path.abspath(__file__))
SRC=os.path.join(BASE,"batch-09.tsv")
FRAGS=["g1","g2","g3"]
PLACE=re.compile(r'%[sSdDfF][0-9]')
OPEN="[LINK="
CLOSE="["+chr(92)+chr(92)+"LINK]"   # [\\LINK]

zh={}
for f in FRAGS:
    p=os.path.join(BASE,"_f09",f+".out.tsv")
