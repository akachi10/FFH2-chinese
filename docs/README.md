...
| 运行环境 | 外部依赖、目录身份、服务生命周期、当前证据和排障方法 | 准备构建/运行环境、覆盖安装或定位“修改未生效”时 | [环境配置](ops/environments.md) · [服务生命周期](ops/services.md) · [环境状态](ops/status.md) · [故障排查](ops/troubleshooting.md) |
...
[环境状态](ops/status.md) → [环境配置](ops/environments.md) → [服务生命周期](ops/services.md) → [故障排查](ops/troubleshooting.md) → [运行验收](testing/runtime-acceptance.md)
